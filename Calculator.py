...
text_area = tk.Text(root, font="55", height=5, width=65, state="disabled")  # Set state to "disabled"
...
def write_to_text_area(value):
    if value == "=":
        try:
            ...
            text_area.configure(state="normal")  # Enable the text area to insert the result
            text_area.delete("1.0", tk.END)
            text_area.insert("end", result)
            text_area.configure(state="disabled")  # Disable the text area again after inserting the result
        ...
        try:
            ...
            text_area.configure(state="normal")  # Enable the text area to show the error message
            text_area.delete("1.0", tk.END)
